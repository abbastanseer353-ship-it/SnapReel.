...
export default function Assistant() {
  ...
  const handleSend = async () => {
    ...
    const updatedWithUser = await appendAiMessage(userId, 'user', text.trim());
    ...
    try {
      ...
      const aiReply = await getAiResponse(text.trim(), historyForApi);

      const updatedWithAi = await appendAiMessage(userId, 'assistant', aiReply);
      setMessages(updatedWithAi);
    } catch (err: any) {
      console.error('AI Error:', err);
      setError(err.message || 'Error calling Gemini API.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && (
          <div className="text-center text-gray-500 mt-20">
            ...
            <p className="text-sm">Ask me anything about video creation or editing.</p>
          </div>
        )}
